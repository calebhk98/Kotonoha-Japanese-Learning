...
console.log("Sudachi morpheme data - checking for base form:\n");
for (const word of testWords) {
  const morphemes = tokenizer.run(word, 'C');
  console.log(`\n${word}:`);
  morphemes.forEach((m, i) => {
    console.log(`  [${i}] surface: "${m.surface}"`);
    console.log(`      pos: ${JSON.stringify(m.part_of_speech)}`);
    console.log(`      dictionary_form: "${(m as any).dictionary_form || 'N/A'}"`);
  });
}
